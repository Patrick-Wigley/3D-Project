...
/* Dynamic Objects 
	These are entities which can move in world-space
*/

/* Entity Objects 
* Objects Utilising Assimp Models - (NOT TERRAINS)
*/
EntityObj::EntityObj(std::vector<float> inital_pos,
...
*/
void EntityObj::SubDraw(Shader& shader, Camera& camera, float CurrentTime)
{
	...
	if (this->pModel->m_HasAnimations)
	{
...
DynamicObj::DynamicObj(std::vector<float> inital_pos, glm::vec3 path,
	Model* model, bool UseIndexBuffer)
	: EntityObj(inital_pos, model, UseIndexBuffer),
	path_vec(path)
{
...
void TerrainObj::AttachBufferData()
{
...
//void DynamicObj::AttachBufferData()
//{
//	/* As in this game I'll be using one VAO, need to keep re-attaching content into array for
//	   obj e.g. Vertices, Normals, Tex-Coords etc.*/
//
...
TerrainObj::TerrainObj(std::vector<float> inital_pos,
...
void TerrainObj::SubDraw(Shader& shader, Camera& camera)
{
	this->AttachBufferData();
	this->Draw(shader, camera);
}
