...
float ToRadian(float x) { return x * (3.14159 / 180.0f); }
float ToDegree(float x) { return x * (180.0f / 3.14159); }
...
void Matrix4f::PrintValues(bool newLine)
{
    newLine ? printf("\n") : NULL;
...
bool Matrix4f::HasSameValue(Matrix4f& otherMatrix)
{
    ...
    for (unsigned int i = 0; i < 4; i++)
    {
        for (unsigned int j = 0; j < 4; j++)
        {
            if (!this->m[i][j] == otherMatrix.m[i][j])
                difference = true;
        }
    }
    return difference;
